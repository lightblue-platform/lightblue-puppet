...
class lightblue::application::migrator (
    ...
    elsif(length($primary_client_certificates) > 1) {
    ...
    $first_client_certificate = merge($client_certificate, $primary_client_certificates[0])
    ...
    class { 'lightblue::application::migrator::daemon':
      ...
      arguments           => {
        name     => $checker_name,
        hostname => $hostname,
        config   => "${first_client_certificate[file_path]}/${first_client_certificate[name]}.properties",
      },
